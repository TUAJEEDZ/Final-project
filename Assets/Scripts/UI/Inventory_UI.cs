...
public class Inventory_UI : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Tab)) // keybind for inventory
        {
            ToggleInventory();
        }
    ...
    public void Remove() // remove item in inventory ui. 
    {
        ...
        if (itemToDrop != null) 
        {
            if(dragSingle)
            {
                ...
                player.inventory.Remove(draggedSlot.slotID);  //remove 1 in slot
            }
            else
            {
                player.DropItem(itemToDrop, player.inventory.slots[draggedSlot.slotID].count);
                player.inventory.Remove(draggedSlot.slotID, player.inventory.slots[draggedSlot.slotID].count); //remove all in slot
            }
